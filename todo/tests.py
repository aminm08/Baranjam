...
class TodoPagesTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.todoListOwnerUserEmail = 'myusername@user.com'
        cls.notOwnerUserEmail = 'testuser2@test.com'
        cls.username = 'testuser2'
        cls.password = 'myusername123'

        cls.todoListOwnerUser = get_user_model().objects.create_user(
            username=cls.todoListOwnerUserEmail,
            email=cls.todoListOwnerUserEmail,
            password=cls.password,
        )
        cls.user2 = get_user_model().objects.create_user(
            username=cls.username,
            email=cls.notOwnerUserEmail,
            password=cls.password,
        )

        cls.todo_list1 = Todo.objects.create(
            name='todo_test',
            user=cls.todoListOwnerUser
        )
    ...
    def test_get_duration_method(self):
        self.assertEqual(Job.objects.get(pk=self.job1.pk).get_duration(), '1h ')
        self.assertEqual(Job.objects.get(pk=self.job2.pk).get_duration(), '30m ')
